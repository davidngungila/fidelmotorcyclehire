...
@section('breadcrumb', 'Accounting \u203A Payments \u203A View Payment')
...
<div class="space-y-6">
  <div class="flex items-center justify-between">
    <div>
      <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Payment #{{ $payment->entry_number }}</h1>
      <p class="text-gray-600 dark:text-gray-400 mt-1">{{ $payment->entry_date->format('F d, Y') }}</p>
    </div>
    <div class="flex items-center gap-2">
      <a href="{{ route('admin.payments.index') }}" class="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-gray-100 hover:bg-gray-200 dark:bg-gray-800 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300 text-sm font-semibold transition-all">
    ...
    <div class="lg:col-span-2 space-y-6">
      <div class="glass rounded-xl p-6">
        ...
        <div class="grid grid-cols-2 gap-4">
          <div>
            <span class="text-sm text-gray-600 dark:text-gray-400">Payment Number:</span>
            <div class="font-mono font-bold text-gray-900 dark:text-white">{{ $payment->entry_number }}</div>
          </div>
          <div>
            <span class="text-sm text-gray-600 dark:text-gray-400">Payment Date:</span>
            <div class="font-semibold text-gray-900 dark:text-white">{{ $payment->entry_date->format('F d, Y') }}</div>
          </div>
          <div>
          ...
          <div>
            ...
            <div class="font-semibold text-gray-900 dark:text-white">{{ $payment->reference }}</div>
          </div>
          ...
          @if($payment->posted_at)
            <div>
              ...
              <div class="font-semibold text-gray-900 dark:text-white">{{ $payment->posted_at->format('M d, Y H:i') }}</div>
            </div>
          @endif
        </div>
      </div>
      ...
      <div class="glass rounded-xl p-6">
        <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Payment Lines</h3>
          ...
          <table class="data-table">
            ...
            <tbody>
              ...
              <tr class="bg-gray-50 dark:bg-gray-800 font-bold">
                ...
                <td class="text-right font-mono text-gray-900 dark:text-white">{{ number_format($payment->total_debit, 2) }}</td>
                <td class="text-right font-mono text-gray-900 dark:text-white">{{ number_format($payment->total_credit, 2) }}</td>
              </tr>
            </tbody>
          </table>
        </div>
      ...
      <div class="glass rounded-xl p-6">
        ...
        <div class="space-y-3">
          <div class="flex justify-between items-center">
            ...
            <span class="font-mono font-bold text-2xl text-primary-700 dark:text-primary-300">{{ number_format($payment->total_credit, 2) }}</span>
          </div>
        </div>
      </div>
        ...
        <div class="glass rounded-xl p-6">
          ...
          <div class="flex items-center gap-3">
            <div class="w-10 h-10 rounded-full bg-primary-100 dark:bg-primary-900/40 flex items-center justify-center">
              <span class="text-primary-700 dark:text-primary-300 font-semibold">{{ strtoupper(substr($payment->createdBy->name, 0, 1)) }}</span>
            </div>
            <div>
              <div class="font-semibold text-gray-900 dark:text-white">{{ $payment->createdBy->name }}</div>
              <div class="text-xs text-gray-500 dark:text-gray-400">{{ $payment->created_at->format('M d, Y H:i') }}</div>
            </div>
          </div>
        </div>
      @endif
