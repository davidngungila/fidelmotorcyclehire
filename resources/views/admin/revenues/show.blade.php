...
@section('breadcrumb', 'Accounting \u203A Revenues \u203A View Revenue')
@section('page_title', 'View Revenue')

@section('content')
<div class="space-y-6">
  <div class="flex items-center justify-between">
    <div>
      <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Revenue #{{ $revenue->entry_number }}</h1>
      <p class="text-gray-600 dark:text-gray-400 mt-1">{{ $revenue->entry_date->format('F d, Y') }}</p>
    </div>
  ...
  <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    <div class="lg:col-span-2 space-y-6">
      <div class="glass rounded-xl p-6">
        <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Revenue Details</h3>
        <div class="grid grid-cols-2 gap-4">
          <div>
            <span class="text-sm text-gray-600 dark:text-gray-400">Revenue Number:</span>
            <div class="font-mono font-bold text-gray-900 dark:text-white">{{ $revenue->entry_number }}</div>
          </div>
          <div>
            <span class="text-sm text-gray-600 dark:text-gray-400">Revenue Date:</span>
            <div class="font-semibold text-gray-900 dark:text-white">{{ $revenue->entry_date->format('F d, Y') }}</div>
          </div>
          <div>
            <span class="text-sm text-gray-600 dark:text-gray-400">Category:</span>
            <div class="font-semibold text-gray-900 dark:text-white">{{ ucfirst($revenue->category) }}</div>
          </div>
          <div>
          ...
          <div>
            ...
            <div class="font-semibold text-gray-900 dark:text-white">{{ $revenue->reference }}</div>
          </div>
          ...
          @if($revenue->posted_at)
            <div>
              <span class="text-sm text-gray-600 dark:text-gray-400">Posted At:</span>
              <div class="font-semibold text-gray-900 dark:text-white">{{ $revenue->posted_at->format('M d, Y H:i') }}</div>
            </div>
          @endif
        </div>
      </div>
      ...
      <div class="glass rounded-xl p-6">
        <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Revenue Lines</h3>
          ...
          <table class="data-table">
            ...
            <tbody>
              ...
              <tr class="bg-gray-50 dark:bg-gray-800 font-bold">
                ...
                <td class="text-right font-mono text-gray-900 dark:text-white">{{ number_format($revenue->total_debit, 2) }}</td>
                <td class="text-right font-mono text-gray-900 dark:text-white">{{ number_format($revenue->total_credit, 2) }}</td>
              </tr>
            </tbody>
          </table>
        </div>
      ...
      <div class="glass rounded-xl p-6">
        ...
        <div class="space-y-3">
          <div class="flex justify-between items-center">
            ...
            <span class="font-mono font-bold text-2xl text-primary-700 dark:text-primary-300">{{ number_format($revenue->total_credit, 2) }}</span>
          </div>
        </div>
      </div>
        ...
        <div class="glass rounded-xl p-6">
          ...
          <div class="flex items-center gap-3">
            <div class="w-10 h-10 rounded-full bg-primary-100 dark:bg-primary-900/40 flex items-center justify-center">
              <span class="text-primary-700 dark:text-primary-300 font-semibold">{{ strtoupper(substr($revenue->createdBy->name, 0, 1)) }}</span>
            </div>
            <div>
              <div class="font-semibold text-gray-900 dark:text-white">{{ $revenue->createdBy->name }}</div>
              <div class="text-xs text-gray-500 dark:text-gray-400">{{ $revenue->created_at->format('M d, Y H:i') }}</div>
            </div>
          </div>
        </div>
      @endif
