...
@section('breadcrumb', 'Motorcycle Inventory \u203A Edit')
@section('page_title', 'Edit Motorcycle')

@section('content')

<div class="max-w-4xl mx-auto">
  <form method="POST" action="{{ route('admin.motorcycles.update', $motorcycle) }}" class="space-y-6">
    @csrf
    @method('PUT')
    ...
    <div class="glass p-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          ...
          <input type="text" name="brand" required value="{{ old('brand', $motorcycle->brand) }}"
                 class="form-input"
        ...
        <div>
          ...
          <input type="text" name="model" required value="{{ old('model', $motorcycle->model) }}"
                 class="form-input"
        ...
        <div>
          ...
          <input type="text" name="engine_number" required value="{{ old('engine_number', $motorcycle->engine_number) }}"
                 class="form-input"
        ...
        <div>
          ...
          <input type="text" name="chassis_number" required value="{{ old('chassis_number', $motorcycle->chassis_number) }}"
                 class="form-input"
        ...
        <div>
          ...
          <input type="text" name="registration_number" value="{{ old('registration_number', $motorcycle->registration_number) }}"
                 class="form-input"
        ...
        <div>
          ...
          <input type="text" name="colour" required value="{{ old('colour', $motorcycle->colour) }}"
                 class="form-input"
        ...
        <div>
          ...
          <input type="number" name="purchase_price" required min="0" step="0.01" value="{{ old('purchase_price', $motorcycle->purchase_price) }}"
                 class="form-input"
                 placeholder="0.00">
        ...
        <div>
          ...
          <input type="number" name="selling_price" min="0" step="0.01" value="{{ old('selling_price', $motorcycle->selling_price) }}"
                 class="form-input"
                 placeholder="0.00">
          @error('selling_price')
            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
        ...
        <div>
          ...
          <select name="status" required class="form-input">
            <option value="Available" {{ $motorcycle->status === 'Available' ? 'selected' : '' }}>Available</option>
            <option value="Assigned" {{ $motorcycle->status === 'Assigned' ? 'selected' : '' }}>Assigned</option>
            <option value="Sold" {{ $motorcycle->status === 'Sold' ? 'selected' : '' }}>Sold</option>
            <option value="Under Repair" {{ $motorcycle->status === 'Under Repair' ? 'selected' : '' }}>Under Repair</option>
          </select>
          @error('status')
            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
          @enderror
        ...
        <div>
          <label class="block text-sm font-medium text-primary-700 dark:text-primary-300 mb-2">Assigned To</label>
          <select name="assigned_to" class="form-input">
            <option value="">-- Not Assigned --</option>
            @foreach(\App\Models\User::all() as $user)
              <option value="{{ $user->id }}" {{ $motorcycle->assigned_to === $user->id ? 'selected' : '' }}>{{ $user->name }}</option>
            @endforeach
          </select>
          @error('assigned_to')
            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
        ...
        <div>
          ...
          <input type="date" name="purchase_date" value="{{ old('purchase_date', $motorcycle->purchase_date?->format('Y-m-d')) }}"
                 class="form-input">
          @error('purchase_date')
            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
          @enderror
        ...
        <div>
          <label class="block text-sm font-medium text-primary-700 dark:text-primary-300 mb-2">Sale Date</label>
          <input type="date" name="sale_date" value="{{ old('sale_date', $motorcycle->sale_date?->format('Y-m-d')) }}"
                 class="form-input">
          @error('sale_date')
            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
          @enderror
      ...
      <div class="mt-6">
        ...
        <textarea name="notes" rows="3"
                  class="form-input"
                  placeholder="Additional notes about this motorcycle...">{{ old('notes', $motorcycle->notes) }}</textarea>
        @error('notes')
          <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
        @enderror
      </div>
    ...
    <div class="flex items-center justify-end gap-3">
      ...
      <button type="submit"
        ...
        <i class="fa-solid fa-save mr-2"></i> Update Motorcycle
      </button>
    </div>
  </form>
</div>
