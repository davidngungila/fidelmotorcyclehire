...
@section('breadcrumb', 'Members \u203A Member Types \u203A Edit')
@section('page_title', 'Edit Member Type')
  ...
  <div class="flex items-center gap-4">
    ...
    <div>
      <p class="text-sm text-primary-600 dark:text-primary-400">
        Edit member type configuration and benefits
      </p>
    </div>
  </div>

  <div class="glass p-6 lg:p-8">
    <form method="POST" action="{{ route('admin.member-types.update', $encryptedId) }}" class="space-y-6">
      @csrf
      @method('PUT')
      ...
      <div class="flex items-center gap-4 pb-6 border-b border-primary-100 dark:border-primary-900/50">
        ...
        <div>
          <h2 class="font-bold text-lg text-primary-900 dark:text-white">Edit Member Type</h2>
          <p class="text-xs mt-0.5 text-primary-600 dark:text-primary-400">Update the member type details and configuration</p>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
        <div class="md:col-span-2">
          ...
          <input type="text" name="name" value="{{ old('name', $memberType->name) }}" required
        ...
        <div>
          ...
          <input type="text" name="code" value="{{ old('code', $memberType->code) }}" required
                 placeholder="e.g. FOUNDER, ORDINARY, SCHOLAR"
        ...
        <div>
          ...
          <input type="number" name="priority" value="{{ old('priority', $memberType->priority) }}" required min="0"
        ...
        <div class="md:col-span-2">
          ...
          <textarea name="description" rows="3"
                    ...
                    class="form-input @error('description') !border-red-400 @enderror">{{ old('description', $memberType->description) }}</textarea>
          @error('description')
            <p class="mt-1.5 text-xs text-red-600 dark:text-red-400"><i class="fa-solid fa-circle-exclamation mr-1 text-[10px]"></i>{{ $message }}</p>
          @enderror
        </div>
      ...
      <div class="pt-5 mt-5 border-t border-primary-100 dark:border-primary-900/50">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
          <div>
            ...
            <input type="number" name="registration_fee" value="{{ old('registration_fee', $memberType->registration_fee) }}" required min="0" step="0.01"
          ...
          <div>
            ...
            <input type="number" name="monthly_contribution" value="{{ old('monthly_contribution', $memberType->monthly_contribution) }}" required min="0" step="0.01"
          ...
          <div>
            ...
            <input type="number" name="min_savings" value="{{ old('min_savings', $memberType->min_savings) }}" required min="0" step="0.01"
      ...
      <div class="pt-5 mt-5 border-t border-primary-100 dark:border-primary-900/50">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
          <div>
            ...
            <input type="number" name="max_loan_multiplier" value="{{ old('max_loan_multiplier', $memberType->max_loan_multiplier) }}" required min="1"
          ...
          <div>
            ...
            <input type="number" name="interest_rate_discount" value="{{ old('interest_rate_discount', $memberType->interest_rate_discount) }}" required min="0" max="100" step="0.01"
          ...
          <div>
            ...
            <div class="grid grid-cols-2 gap-3">
              <label class="flex items-center justify-center gap-2 px-4 py-3 rounded-xl border-2 cursor-pointer transition-all text-xs font-semibold
                ...
                <input type="radio" name="can_vote" value="1" class="hidden" {{ old('can_vote', $memberType->can_vote) ? 'checked' : '' }}>
              ...
              <label class="flex items-center justify-center gap-2 px-4 py-3 rounded-xl border-2 cursor-pointer transition-all text-xs font-semibold
                ...
                <input type="radio" name="can_vote" value="0" class="hidden" {{ old('can_vote', $memberType->can_vote) === 0 ? 'checked' : '' }}>
          ...
          <div>
            ...
            <div class="grid grid-cols-2 gap-3">
              <label class="flex items-center justify-center gap-2 px-4 py-3 rounded-xl border-2 cursor-pointer transition-all text-xs font-semibold
                           border-primary-200 dark:border-primary-900/60 bg-primary-50 dark:bg-primary-900/30 text-primary-700 dark:text-primary-300
                           has-[:checked]:border-green-500 has-[:checked]:bg-green-50 dark:has-[:checked]:bg-green-900/30 has-[:checked]:text-green-700 dark:has-[:checked]:text-green-300">
                <input type="radio" name="can_hold_office" value="1" class="hidden" {{ old('can_hold_office', $memberType->can_hold_office) ? 'checked' : '' }}>
                <i class="fa-solid fa-check text-[12px]"></i> Yes
              </label>
              <label class="flex items-center justify-center gap-2 px-4 py-3 rounded-xl border-2 cursor-pointer transition-all text-xs font-semibold
                           border-primary-200 dark:border-primary-900/60 bg-primary-50 dark:bg-primary-900/30 text-primary-700 dark:text-primary-300
                           has-[:checked]:border-red-500 has-[:checked]:bg-red-50 dark:has-[:checked]:bg-red-900/30 has-[:checked]:text-red-700 dark:has-[:checked]:text-red-300">
                <input type="radio" name="can_hold_office" value="0" class="hidden" {{ old('can_hold_office', $memberType->can_hold_office) === 0 ? 'checked' : '' }}>
                <i class="fa-solid fa-xmark text-[12px]"></i> No
              </label>
            </div>
          </div>
      ...
      <div class="pt-5 mt-5 border-t border-primary-100 dark:border-primary-900/50">
        ...
        <div class="grid grid-cols-2 gap-3">
          <label class="flex items-center justify-center gap-2 px-4 py-3 rounded-xl border-2 cursor-pointer transition-all text-xs font-semibold
                       border-primary-200 dark:border-primary-900/60 bg-primary-50 dark:bg-primary-900/30 text-primary-700 dark:text-primary-300
                       has-[:checked]:border-green-500 has-[:checked]:bg-green-50 dark:has-[:checked]:bg-green-900/30 has-[:checked]:text-green-700 dark:has-[:checked]:text-green-300">
            <input type="radio" name="status" value="active" class="hidden" {{ old('status', $memberType->status) === 'active' ? 'checked' : '' }}>
          ...
          <label class="flex items-center justify-center gap-2 px-4 py-3 rounded-xl border-2 cursor-pointer transition-all text-xs font-semibold
            ...
            <input type="radio" name="status" value="inactive" class="hidden" {{ old('status', $memberType->status) === 'inactive' ? 'checked' : '' }}>
      ...
      <div class="pt-6 mt-6 border-t border-primary-100 dark:border-primary-900/50 flex flex-col sm:flex-row items-center justify-end gap-3">
        ...
        <button type="submit"
          ...
          <i class="fa-solid fa-save mr-1.5 text-[13px]"></i> Update Member Type
        </button>
      </div>
    </form>
  </div>
