...
<div class="space-y-6">
  ...
  <div class="glass p-5">
      ...
      <table class="w-full">
        <thead>
          <tr class="border-b border-primary-700/20">
            ...
            <th class="text-left py-3 px-4 text-xs font-semibold text-primary-600">Type</th>
            ...
            <th class="text-left py-3 px-4 text-xs font-semibold text-primary-600">Transaction Date</th>
        ...
        <tbody>
          ...
          <tr class="border-b border-primary-700/10 hover:bg-primary-800/30 transition-colors">
            <td class="py-3 px-4 text-sm font-medium text-white">{{ $transaction->user->name ?? 'N/A' }}</td>
            <td class="py-3 px-4 text-sm text-primary-300">{{ $transaction->shareProduct->name ?? 'N/A' }}</td>
            ...
            <td class="py-3 px-4 text-center">
              <div class="flex items-center justify-center gap-2">
                <a href="{{ route('admin.share-transactions.show', $transaction) }}" class="text-primary-400 hover:text-white transition-colors">
                  <i class="fa-solid fa-eye text-sm"></i>
                </a>
                <a href="{{ route('admin.share-transactions.edit', $transaction) }}" class="text-primary-400 hover:text-white transition-colors">
                  <i class="fa-solid fa-edit text-sm"></i>
                </a>
                <form action="{{ route('admin.share-transactions.destroy', $transaction) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this share transaction?');">
                  @csrf
                  @method('DELETE')
          ...
          @empty
          <tr>
            <td colspan="8" class="py-8 text-center text-primary-400 text-sm">No share transactions found</td>
          </tr>
          @endforelse
        </tbody>
      </table>
    ...
    <div class="flex items-center justify-between mt-5 pt-5 border-t border-primary-700/20">
      <span class="text-xs text-primary-400">Showing {{ $shareTransactions->firstItem() }} to {{ $shareTransactions->lastItem() }} of {{ $shareTransactions->total() }} results</span>
