...
<div class="space-y-6">
  ...
  <div class="bg-white dark:bg-dark-card rounded-xl shadow-sm border border-primary-100 dark:border-primary-800 p-5">
      ...
      <table class="w-full">
        <thead>
          <tr class="bg-primary-50 dark:bg-primary-900/20">
            <th class="text-left py-3 px-4 text-xs font-semibold text-primary-600 dark:text-primary-400">From User</th>
            <th class="text-left py-3 px-4 text-xs font-semibold text-primary-600 dark:text-primary-400">To User</th>
            ...
            <th class="text-left py-3 px-4 text-xs font-semibold text-primary-600 dark:text-primary-400">Transfer Date</th>
        ...
        <tbody>
          ...
          <tr class="border-b border-primary-100 dark:border-primary-800 hover:bg-primary-50 dark:hover:bg-primary-900/10 transition-colors">
            <td class="py-3 px-4 text-sm font-medium text-primary-900 dark:text-white">{{ $transfer->fromUser->name ?? 'N/A' }}</td>
            <td class="py-3 px-4 text-sm text-primary-700 dark:text-primary-300">{{ $transfer->toUser->name ?? 'N/A' }}</td>
            <td class="py-3 px-4 text-sm text-primary-700 dark:text-primary-300">{{ $transfer->shareCertificate->certificate_number ?? 'N/A' }}</td>
            <td class="py-3 px-4 text-sm text-primary-700 dark:text-primary-300">{{ $transfer->number_of_shares }}</td>
            <td class="py-3 px-4 text-sm text-primary-700 dark:text-primary-300">{{ $transfer->transfer_date ? $transfer->transfer_date->format('M d, Y') : 'N/A' }}</td>
            <td class="py-3 px-4">
            ...
            <td class="py-3 px-4 text-center">
              <div class="flex items-center justify-center gap-2">
                <a href="{{ route('admin.share-transfers.show', app('App\Services\EncryptedIdService')->encrypt($transfer->id)) }}" class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-teal-100 hover:bg-teal-200 dark:bg-teal-900/40 dark:hover:bg-teal-900/60 text-teal-700 dark:text-teal-300 text-xs font-medium transition-colors">
                  <i class="fa-solid fa-eye text-[10px]"></i> View
                </a>
                <a href="{{ route('admin.share-transfers.edit', app('App\Services\EncryptedIdService')->encrypt($transfer->id)) }}" class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-primary-100 hover:bg-primary-200 dark:bg-primary-900/40 dark:hover:bg-primary-900/60 text-primary-700 dark:text-primary-300 text-xs font-medium transition-colors">
                  <i class="fa-solid fa-edit text-[10px]"></i> Edit
                </a>
                <form action="{{ route('admin.share-transfers.destroy', app('App\Services\EncryptedIdService')->encrypt($transfer->id)) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this share transfer?');">
                  @csrf
                  @method('DELETE')
          ...
          @empty
          <tr>
            <td colspan="7" class="py-8 text-center text-primary-600 dark:text-primary-400 text-sm">No share transfers found</td>
          </tr>
          @endforelse
        </tbody>
      </table>
    ...
    <div class="flex items-center justify-between mt-5 pt-5 border-t border-primary-100 dark:border-primary-800">
      <span class="text-xs text-primary-600 dark:text-primary-400">Showing {{ $shareTransfers->firstItem() }} to {{ $shareTransfers->lastItem() }} of {{ $shareTransfers->total() }} results</span>
