  ...
  <div class="flex items-center gap-4">
    ...
    <div>
      <p class="text-sm text-primary-600 dark:text-primary-400">
        Edit member information and details
      </p>
    </div>
  </div>
  ...
  <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
    ...
    <div class="lg:col-span-3">
      <div class="glass p-6 lg:p-8">
        <form method="POST" action="{{ route('admin.users.update', $user->id) }}" id="memberForm" class="space-y-6">
          @csrf
          @method('PUT')
          ...
          <div id="content-basic" class="tab-content">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div>
                ...
                <select name="member_type_id" class="form-select @error('member_type_id') !border-red-400 @enderror">
                  ...
                  @foreach($memberTypes as $type)
                    <option value="{{ $type->id }}" {{ old('member_type_id', $user->member_type_id) == $type->id ? 'selected' : '' }}>{{ $type->name }}</option>
                  @endforeach
                </select>
              ...
              <div>
                ...
                <input type="text" name="passport_license" value="{{ old('passport_license', $user->passport_license) }}"
              ...
              <div>
                ...
                <input type="text" name="member_number" value="{{ old('member_number', $user->member_number) }}" readonly
              ...
              <div>
                ...
                <input type="date" name="registration_date" value="{{ old('registration_date', $user->registration_date ? $user->registration_date->format('Y-m-d') : '') }}"
          ...
          <div id="content-contact" class="tab-content hidden">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <div>
                ...
                <input type="tel" name="alternative_phone" value="{{ old('alternative_phone', $user->alternative_phone) }}"
                       placeholder="Optional"
              ...
              <div>
                ...
                <input type="text" name="street_village" value="{{ old('street_village', $user->street_village) }}"
              ...
              <div class="md:col-span-2">
                ...
                <textarea name="physical_address" rows="2"
                          ...
                          class="form-input @error('physical_address') !border-red-400 @enderror">{{ old('physical_address', $user->physical_address) }}</textarea>
          ...
          <div id="content-membership" class="tab-content hidden">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div>
                ...
                <input type="text" name="branch" value="{{ old('branch', $user->branch) }}" required
              ...
              <div>
                ...
                <input type="text" name="membership_category" value="{{ old('membership_category', $user->membership_category) }}"
              ...
              <div>
                ...
                <input type="text" name="employer_business" value="{{ old('employer_business', $user->employer_business) }}"
              ...
              <div>
                ...
                <input type="number" name="monthly_income" value="{{ old('monthly_income', $user->monthly_income) }}" min="0" step="0.01"
              ...
              <div>
                ...
                <input type="number" name="joining_fee" value="{{ old('joining_fee', $user->joining_fee) }}" min="0" step="0.01"
              ...
              <div>
                ...
                <input type="number" name="shares_purchased" value="{{ old('shares_purchased', $user->shares_purchased) }}" min="0" step="0.01"
              ...
              <div class="md:col-span-2">
                ...
                <input type="number" name="initial_savings" value="{{ old('initial_savings', $user->initial_savings) }}" min="0" step="0.01"
          ...
          <div id="content-account" class="tab-content hidden">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <div>
                <label class="form-label uppercase tracking-wider text-primary-700 dark:text-primary-300">Password <span class="font-normal normal-case opacity-70">(leave blank to keep current)</span></label>
              ...
              <div>
                ...
                <div class="grid grid-cols-2 gap-3">
                  <label class="flex items-center justify-center gap-2 px-4 py-3 rounded-xl border-2 cursor-pointer transition-all text-xs font-semibold
                    ...
                    <input type="radio" name="email_verified" value="1" class="hidden" {{ old('email_verified', $user->email_verified) ? 'checked' : '' }}>
                  ...
                  <label class="flex items-center justify-center gap-2 px-4 py-3 rounded-xl border-2 cursor-pointer transition-all text-xs font-semibold
                    ...
                    <input type="radio" name="email_verified" value="0" class="hidden" {{ old('email_verified', $user->email_verified) === 0 ? 'checked' : '' }}>
              ...
              <div>
                ...
                <div class="grid grid-cols-2 gap-3">
                  <label class="flex items-center justify-center gap-2 px-4 py-3 rounded-xl border-2 cursor-pointer transition-all text-xs font-semibold
                               border-primary-200 dark:border-primary-900/60 bg-primary-50 dark:bg-primary-900/30 text-primary-700 dark:text-primary-300
                               has-[:checked]:border-green-500 has-[:checked]:bg-green-50 dark:has-[:checked]:bg-green-900/30 has-[:checked]:text-green-700 dark:has-[:checked]:text-green-300">
                    <input type="radio" name="phone_verified" value="1" class="hidden" {{ old('phone_verified', $user->phone_verified) ? 'checked' : '' }}>
                    <i class="fa-solid fa-check text-[12px]"></i> Yes
                  </label>
                  <label class="flex items-center justify-center gap-2 px-4 py-3 rounded-xl border-2 cursor-pointer transition-all text-xs font-semibold
                               border-primary-200 dark:border-primary-900/60 bg-primary-50 dark:bg-primary-900/30 text-primary-700 dark:text-primary-300
                               has-[:checked]:border-red-500 has-[:checked]:bg-red-50 dark:has-[:checked]:bg-red-900/30 has-[:checked]:text-red-700 dark:has-[:checked]:text-red-300">
                    <input type="radio" name="phone_verified" value="0" class="hidden" {{ old('phone_verified', $user->phone_verified) === 0 ? 'checked' : '' }}>
                    <i class="fa-solid fa-xmark text-[12px]"></i> No
                  </label>
                </div>
              </div>
          ...
          <div id="content-documents" class="tab-content hidden">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div>
                ...
                @if($user->documents && $user->documents->passport_photo)
                  <p class="mt-1 text-xs text-primary-600 dark:text-primary-400">Current: {{ $user->documents->passport_photo }}</p>
                @endif
              ...
              <div>
                ...
                @if($user->documents && $user->documents->national_id_copy)
                  <p class="mt-1 text-xs text-primary-600 dark:text-primary-400">Current: {{ $user->documents->national_id_copy }}</p>
                @endif
              ...
              <div>
                ...
                @if($user->documents && $user->documents->signature)
                  <p class="mt-1 text-xs text-primary-600 dark:text-primary-400">Current: {{ $user->documents->signature }}</p>
                @endif
                @error('signature')
                  <p class="mt-1.5 text-xs text-red-600 dark:text-red-400"><i class="fa-solid fa-circle-exclamation mr-1 text-[10px]"></i>{{ $message }}</p>
                @enderror
              ...
              <div>
                ...
                @if($user->documents && $user->documents->other_attachments)
                  <p class="mt-1 text-xs text-primary-600 dark:text-primary-400">Current: {{ $user->documents->other_attachments }}</p>
                @endif
            ...
            <div class="grid grid-cols-1 gap-5">
              <div>
                ...
                <textarea name="notes" rows="4"
                          ...
                          class="form-input @error('notes') !border-red-400 @enderror">{{ old('notes', $user->notes) }}</textarea>
                @error('notes')
                  <p class="mt-1.5 text-xs text-red-600 dark:text-red-400"><i class="fa-solid fa-circle-exclamation mr-1 text-[10px]"></i>{{ $message }}</p>
                @enderror
              </div>
              ...
              <div>
                ...
                <input type="text" name="tags" value="{{ old('tags', is_array($user->tags) ? implode(',', $user->tags) : $user->tags) }}"
      ...
      <div class="glass p-6 rounded-2xl">
        ...
        <div class="space-y-4">
          <div>
            ...
            <p class="text-sm font-semibold text-primary-900 dark:text-white font-mono">{{ $user->member_number ?? 'N/A' }}</p>
          </div>
          <div>
            <p class="text-xs text-primary-600 dark:text-primary-400">Registration Date</p>
            <p class="text-sm font-semibold text-primary-900 dark:text-white">{{ $user->registration_date ? $user->registration_date->format('M d, Y') : 'N/A' }}</p>
          </div>
          <div>
