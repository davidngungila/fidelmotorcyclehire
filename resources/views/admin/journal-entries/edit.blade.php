...
@section('page_title', 'Edit Journal Entry')

@section('content')
<div class="space-y-6">
  <div class="flex items-center justify-between">
    <div>
      <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Edit Journal Entry #{{ $journalEntry->entry_number }}</h1>
    ...
    <form action="{{ route('admin.journal-entries.update', $journalEntry->id) }}" method="POST" id="journalEntryForm" class="space-y-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div>
          ...
          <input type="date" name="entry_date" value="{{ old('entry_date', $journalEntry->entry_date->format('Y-m-d')) }}" required
        ...
        <div>
          ...
          <select name="entry_type" required
            ...
            <option value="manual" {{ $journalEntry->entry_type === 'manual' ? 'selected' : '' }}>Manual</option>
        ...
        <div>
          ...
          <select name="financial_period_id"
            ...
            @foreach($financialPeriods as $period)
              <option value="{{ $period->id }}" {{ old('financial_period_id', $journalEntry->financial_period_id) == $period->id ? 'selected' : '' }}>
      ...
      <div>
        ...
        <input type="text" name="description" value="{{ old('description', $journalEntry->description) }}" required
          class="form-input py-2.5 px-4 w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-dark-card text-gray-900 dark:text-white focus:ring-2 focus:ring-primary-500 focus:border-transparent"
      ...
      <div>
        ...
        <input type="text" name="reference" value="{{ old('reference', $journalEntry->reference) }}"
          class="form-input py-2.5 px-4 w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-dark-card text-gray-900 dark:text-white focus:ring-2 focus:ring-primary-500 focus:border-transparent"
      ...
      <div>
        ...
        <div id="linesContainer" class="space-y-3">
          @foreach($journalEntry->lines as $index => $line)
            <div class="line-item bg-white dark:bg-dark-card rounded-lg p-4 border border-gray-200 dark:border-gray-700" data-line="{{ $index }}">
              <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div class="md:col-span-2">
                  <label class="block text-xs font-semibold text-gray-700 dark:text-gray-300 mb-1">Account *</label>
                  <select name="lines[{{ $index }}][account_id]" required onchange="updateTotals()"
                    ...
                    @foreach($accounts as $acc)
                      <option value="{{ $acc->id }}" data-type="{{ $acc->account_type }}" {{ $line->account_id == $acc->id ? 'selected' : '' }}>
                ...
                <div>
                  ...
                  <input type="number" name="lines[{{ $index }}][debit_amount]" step="0.01" min="0" value="{{ $line->debit_amount }}" onchange="updateTotals()"
                ...
                <div>
                  ...
                  <input type="number" name="lines[{{ $index }}][credit_amount]" step="0.01" min="0" value="{{ $line->credit_amount }}" onchange="updateTotals()"
                    class="form-input py-2 px-3 w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-dark-card text-gray-900 dark:text-white text-sm focus:ring-2 focus:ring-primary-500 focus:border-transparent">
                </div>
              </div>
              <div class="mt-3">
              ...
              <button type="button" onclick="removeLine({{ $index }})" class="mt-3 text-xs text-red-600 hover:text-red-700 dark:text-red-400 dark:hover:text-red-300">
        ...
        <div class="mt-4 p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
          <div class="grid grid-cols-2 gap-4">
            <div>
              ...
              <div id="totalDebit" class="text-xl font-bold text-gray-900 dark:text-white">{{ number_format($journalEntry->total_debit, 2) }}</div>
            </div>
            <div>
              <span class="text-sm text-gray-600 dark:text-gray-400">Total Credit:</span>
              <div id="totalCredit" class="text-xl font-bold text-gray-900 dark:text-white">{{ number_format($journalEntry->total_credit, 2) }}</div>
            </div>
          </div>
      ...
      <div class="flex items-center justify-end gap-3 pt-4 border-t border-gray-200 dark:border-gray-700">
        <a href="{{ route('admin.journal-entries.index') }}" class="px-6 py-2.5 rounded-lg bg-gray-100 hover:bg-gray-200 dark:bg-gray-800 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300 text-sm font-semibold transition-all">
          Cancel
        </a>
...
@push('scripts')
<script>
let lineCount = {{ $journalEntry->lines->count() }};
