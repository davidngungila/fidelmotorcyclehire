      ...
      <div x-show="activeTab === 'whatsapp'" x-transition:enter="transition ease-out duration-200"
        ...
        <div class="bg-white dark:bg-dark-card rounded-xl shadow-sm border border-primary-100 dark:border-primary-800 p-6">
          ...
          <div x-data="{ 
            ...
            <button type="button" @click="checkConnection()" :disabled="loading"
            ...
            <div x-show="status" x-transition class="mt-3 p-4 rounded-xl border" 
                 :class="status && status.success ? 'bg-green-50 dark:bg-green-900/20 border-green-200 dark:border-green-800' : 'bg-red-50 dark:bg-red-900/20 border-red-200 dark:border-red-800'">
              <div class="flex items-start gap-3">
                <i :class="status && status.success ? 'fa-solid fa-circle-check text-green-600 dark:text-green-400' : 'fa-solid fa-circle-xmark text-red-600 dark:text-red-400'" class="mt-0.5"></i>
                <div class="flex-1">
                  <p class="text-sm font-semibold" :class="status && status.success ? 'text-green-800 dark:text-green-200' : 'text-red-800 dark:text-red-200'" x-text="status ? status.message : ''"></p>
                  <template x-if="status && status.active_session">
